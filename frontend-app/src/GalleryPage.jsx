...
export default function GalleryPage({ contract, walletAddress }) {
      ...
      await Promise.all(
        projects.map(async project => {
          ...
          try {
            ...
            console.log(`[Gallery] Fetching metadata for project ${project.projectId} from:`, url);
            const res = await fetch(url, { cache: "reload" });
            if (res.ok) {
              const meta = await res.json();
              newMeta[project.projectId] = meta;
              console.log(`[Gallery] Metadata for ${project.projectId}:`, meta);
            } else {
              console.warn(`[Gallery] Metadata fetch failed for ${url}, status: ${res.status}`);
            }
          } catch (err) {
            console.warn(`[Gallery] Metadata fetch error for CID:`, project.ipfsCID, err);
          }
        })
      );
      setIpfsMeta(newMeta);
  ...
  const getMeta = (project) => {
    ...
    // Log the image link if present
    if (meta?.image) {
      const imageUrl = meta.image.startsWith("ipfs://")
        ? ipfsToHttp(meta.image)
        : meta.image;
      console.log(`[Gallery] Project ${project.projectId} - Resolved image:`, imageUrl);
    } else {
      console.log(`[Gallery] Project ${project.projectId} - No image in metadata.`);
    }
    return {
      name: meta?.name || project.projectName,
      description: meta?.description,
      image: meta?.image ? (meta.image.startsWith("ipfs://") ? ipfsToHttp(meta.image) : meta.image) : null,
